...
from .exec_eval import eval_exec_match_qatch
import signal
...
def compute_score(solution_str: str, 
    ...
    db_path = os.path.join('data/NL2SQL/BIRD/train/train_databases', db_name, db_name + '.sqlite')
    # 初始化三个指标的实例
    ...
    if format_correct and pred_sql:
        ...
        try:
            # 假设 eval_exec_match 返回目标表格和预测表格
            exec_status, target_table, prediction_table = func_timeout(
                timeout=30,
                func=eval_exec_match_qatch,
            ...
            # print("[Exec status]: Unexecutable")
        ...
        if exec_status == 'Unexecutable':
            ...
            # 计算三个指标的得分
            if target_table and prediction_table:
                precision_score = cell_precision.run_metric(target_table, prediction_table)
                cardinality_score = tuple_cardinality.run_metric(target_table, prediction_table)
                recall_score = cell_recall.run_metric(target_table, prediction_table)
                # 三个指标得分求和
                result_score = precision_score + cardinality_score + recall_score
            else:
                result_score = 0
                precision_score = 0
                cardinality_score = 0
            ...
            # print(f"[Cardinality score]: {cardinality_score}")
            # print(f"[Recall score]: {recall_score}")
        elif exec_status == 'Match':
            exec_score = EXEC_REWARD
            result_score = RESULT_REWARD
            # 原结果匹配逻辑
            # exec_status = 'Match' if exec_score == 3 else 'Mismatch'
